...
public class Controller implements IController {
	...
	private ISessionBean session = null;

	private BeanHandler beanHandler = null;

	private IProfileHandler profileHandler = null;
	...
	public void saveBeanToRequest(IBean bean, HttpServletRequest request) {
	...
	public ISessionBean getSession(HttpServletRequest request) {
		...
		try {
			session = (ISessionBean) request.getSession().getAttribute(
					classSessione);
			if (session == null) {
				session = (ISessionBean) Class.forName(classSessione)
						.newInstance();
	...
	public IProfileHandler getProfileHandler() {
		...
		if (profileHandler == null) {
			...
			if (className != null) {
				try {
					profileHandler = (IProfileHandler) Class.forName(className)
							.newInstance();
