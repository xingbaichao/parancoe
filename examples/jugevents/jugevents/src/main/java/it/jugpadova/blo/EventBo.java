...
public class EventBo {
	...
	public List<Event> search(EventSearch eventSearch) {
		...
		try {
			DetachedCriteria eventCriteria = DetachedCriteria
			...
			if (StringUtils.isNotBlank(eventSearch.getJugName())
				...
				DetachedCriteria ownerCriteria = eventCriteria
						.createCriteria("owner.jug");
				if (StringUtils.isNotBlank(eventSearch.getJugName())) {
					ownerCriteria.add(Restrictions.ilike("name", eventSearch
							.getJugName(), MatchMode.ANYWHERE));
				}
				if (StringUtils.isNotBlank(eventSearch.getCountry())
						|| StringUtils.isNotBlank(eventSearch.getContinent())) {
					DetachedCriteria countryCriteria = ownerCriteria
					...
					if (StringUtils.isNotBlank(eventSearch.getContinent())) {
						DetachedCriteria continentCriteria = countryCriteria
								.createCriteria("continent");
						continentCriteria
								.add(Restrictions.ilike("name", eventSearch
										.getContinent(), MatchMode.ANYWHERE));
					}
				}
			}
	...
	private void sendConfirmationEmail(final Event event,
		...
		MimeMessagePreparator preparator = new MimeMessagePreparator() {
			...
			public void prepare(MimeMessage mimeMessage) throws Exception {
				...
				model.put("confirmationCode", URLEncoder.encode(participant
	...
	public void copyDirectionsFromEvent(long eventId) {
		...
		if (event != null) {
			...
			util.setValue("directionsPreview", FilterBo.filterText(event
					.getDirections(), event.getFilter(), false));
	...
	public Participant confirmParticipant(String email, String confirmationCode) {
		...
		List<Participant> participants = dao
				.findByEmailAndConfirmationCodeAndConfirmed(email,
	...
	public void updateBadgePanel(String continent, String country,
		...
		java.lang.String baseUrl = "http://" + req.getServerName() + ":"
		...
		boolean showDescription = java.lang.Boolean
				.parseBoolean(jebShowDescription);
	...
	public String getBadgePageCode(String baseUrl, String continent,
		...
		result.append("<script type=\"text/javascript\" src=\"")
				.append(baseUrl).append("/event/badge.html");
		try {
	...
	public String getBadgeHtmlCode(List<Event> events, DateFormat dateFormat,
		...
		if ("simple".equals(badgeStyle)) {
			...
			result
					.append(".jeb_event_odd {background-color: rgb(90%, 90%, 90%); padding: 4px;}\n");
			result
					.append(".jeb_event_even {background-color: white; padding: 4px;}\n");
	...
	public List<NewsMessage> buildNewsMessages(String baseUrl) {
		...
		for (Event event : upcomings) {
			messages.add(new NewsMessage(NewsMessage.TYPE_UPCOMING_EVENT, event
					.getStartDate(), event, baseUrl));
		}
		...
		for (Event event : newEvents) {
			if (!upcomings.contains(event)) {
				messages.add(new NewsMessage(NewsMessage.TYPE_NEW_EVENT, event
						.getStartDate(), event, baseUrl));
			}
		}
		return messages;
