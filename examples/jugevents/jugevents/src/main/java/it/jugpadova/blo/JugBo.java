...
public class JugBo {
	...
	public void updateJugList(String kmlUrl) throws Exception {
		...
		Element document = kml
				.getFirstChildElement("Document", EARTH_NAMESPACE);
		Elements continents = document.getChildElements("Folder",
				EARTH_NAMESPACE);
		for (int i = 0; i < continents.size(); i++) {
			...
			if ("Oceania".equals(continentName)) {
				...
				logger
						.info("Substituted continent name: Oceania with Australia");
			}
			Continent continentPo = continentDao.findByName(continentName);
			if (continentPo != null) {
				logger.info("Loading " + continentName + " countries.");
				Elements countries = continent.getChildElements("Folder",
						EARTH_NAMESPACE);
				for (int j = 0; j < countries.size(); j++) {
					Element country = countries.get(j);
					String countryName = country.getFirstChildElement("name",
							EARTH_NAMESPACE).getValue();
					Country countryPo = countryDao
							.findByEnglishName(countryName);
					if (countryPo != null) {
						logger.info("Loading " + countryName + " JUGs.");
						Elements placemarks = country.getChildElements(
								"Placemark", EARTH_NAMESPACE);
						for (int k = 0; k < placemarks.size(); k++) {
							Element placemark = placemarks.get(k);
							String jugName = placemark.getFirstChildElement(
									"name", EARTH_NAMESPACE).getValue();
							String description = placemark
									.getFirstChildElement("description",
											EARTH_NAMESPACE).getValue();
							Element point = placemark.getFirstChildElement(
									"Point", EARTH_NAMESPACE);
							String coordinatesStr = point.getFirstChildElement(
									"coordinates", EARTH_NAMESPACE).getValue();
							String[] coordinatesArr = coordinatesStr.split(",");
							Double longitude = Double
									.parseDouble(coordinatesArr[0]);
							Double latitude = Double
									.parseDouble(coordinatesArr[1]);
							JUG jug = jugDao.findByName(jugName);
							if (jug != null && jug.isModifiedKmlData() != null
									&& jug.isModifiedKmlData().booleanValue()) {
								logger
										.info("Skipping updating of "
												+ jugName
	...
	private Boolean evaluateModifiedKmlDate(JUG newJUG, JUG oldJUG) {
		return (newJUG.getLongitude() != null && !newJUG.getLongitude().equals(
				oldJUG.getLongitude()))
				|| (newJUG.getLatitude() != null && !newJUG.getLatitude()
						.equals(oldJUG.getLatitude()))
				|| (newJUG.getInfos() != null && !newJUG.getInfos().equals(
						oldJUG.getInfos()));
	}
