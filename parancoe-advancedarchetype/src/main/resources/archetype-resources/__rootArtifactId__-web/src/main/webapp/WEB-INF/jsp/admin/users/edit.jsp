...
<div><a href="${symbol_dollar}{cpr}/admin/users" class="action action-back"><spring:message code="back" text="?back?"/></a></div>
<div class="form-container">
    <form:form commandName="userBean" method="POST" action="${symbol_dollar}{cpr}/admin/users/${symbol_dollar}{userBean.user.id}">
        <div>
            ...
            <div>
                <c:forEach items="${symbol_dollar}{userBean.authorityCheckBoxes}" var="authorityCheckBox" varStatus="loopStatus">
                    <spring:bind path="userBean.authorityCheckBoxes[${symbol_dollar}{loopStatus.index}].checked">
                        ...
                        <input type="checkbox" id="<c:out value="${symbol_dollar}{status.expression}"/>" name="<c:out value="${symbol_dollar}{status.expression}"/>" value="true" <c:if test="${symbol_dollar}{status.value}">checked</c:if>/><label for="<c:out value="${symbol_dollar}{status.expression}"/>">${symbol_dollar}{authorityCheckBox.authority.role}</label><br/>
                    </spring:bind>
                </c:forEach>
            </div>
        </div>
