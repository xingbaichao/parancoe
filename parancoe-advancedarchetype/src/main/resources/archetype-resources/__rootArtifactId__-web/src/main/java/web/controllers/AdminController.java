...
public class AdminController {
    ...
    private String colourLog(String log) {
        ...
        if (log == null) {
            ...
            lines = log.split("[${symbol_escape}${symbol_escape}n${symbol_escape}${symbol_escape}r]");
        }
        for (int i = 0; i < lines.length; i++) {
            if (lines[i].indexOf("[ERROR]") != -1) {
                lines[i] = "<span class=${symbol_escape}"log_error${symbol_escape}">" + lines[i] + "</span>";
            }
            if (lines[i].indexOf("[WARN]") != -1) {
                lines[i] = "<span class=${symbol_escape}"log_warn${symbol_escape}">" + lines[i] + "</span>";
            }
