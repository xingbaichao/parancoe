 ...
 */
public class ParancoeAnnotationMethodHandlerAdapter extends WebContentGenerator
    ...
    /**
     * Set the validator to use on @ModelAttribute parameter of methods
    ...
    private ModelAndView invokeHandlerMethod(
        ...
        try {
            ServletHandlerMethodResolver methodResolver = getMethodResolver(
                    handler);
    ...
    protected ServletRequestDataBinder createBinder(
            HttpServletRequest request, Object target, String objectName) throws
            Exception {
    ...
    private ServletHandlerMethodResolver getMethodResolver(Object handler) {
        ...
        ServletHandlerMethodResolver resolver = this.methodResolverCache.get(
                handlerClass);
        if (resolver == null) {
    ...
    private class ServletHandlerMethodResolver extends HandlerMethodResolver {
        ...
        public Method resolveHandlerMethod(HttpServletRequest request) throws
            ...
            Map<RequestMappingInfo, Method> targetHandlerMethods =
                    new LinkedHashMap<RequestMappingInfo, Method>();
            Map<RequestMappingInfo, String> targetPathMatches =
                    new LinkedHashMap<RequestMappingInfo, String>();
                    ...
                    if (oldMappedMethod != null && oldMappedMethod !=
                        ...
                        if (methodNameResolver != null &&
                            ...
                            if (resolvedMethodName == null) {
                                resolvedMethodName =
                                        methodNameResolver.getHandlerMethodName(
                                        request);
                            }
                            if (!resolvedMethodName.equals(
                                    oldMappedMethod.getName())) {
                ...
                for (RequestMappingInfo mapping : targetHandlerMethods.keySet()) {
                    ...
                    if (bestMappingMatch == null) {
                        ...
                        if (isBetterPathMatch(mappedPath, bestPathMatch,
                                lookupPath) ||
                                (!isBetterPathMatch(bestPathMatch, mappedPath,
                                lookupPath) &&
        ...
        private boolean isPathMatch(String mappedPath, String lookupPath) {
            if (mappedPath.equals(lookupPath) || pathMatcher.match(mappedPath,
                    lookupPath)) {
                return true;
            }
            ...
            return (!mappedPath.startsWith("/") &&
                    (lookupPath.endsWith(mappedPath) || pathMatcher.match(
                    "/**/" + mappedPath, lookupPath) ||
                    (!hasSuffix && pathMatcher.match("/**/" + mappedPath + ".*",
                    lookupPath))));
        }

        private boolean checkParameters(RequestMappingInfo mapping,
                HttpServletRequest request) {
            return ServletAnnotationMappingUtils.checkRequestMethod(
                    mapping.methods, request) &&
                    ServletAnnotationMappingUtils.checkParameters(mapping.params,
                    request);
        }
        ...
        private boolean isBetterPathMatch(String mappedPath,
            ...
            return (mappedPath != null && (mappedPathToCompare == null ||
                    mappedPath.equals(lookupPath) ||
                    mappedPathToCompare.length() < mappedPath.length()));
        }

        private boolean isBetterMethodMatch(RequestMappingInfo mapping,
                RequestMappingInfo mappingToCompare) {
            return (mappingToCompare.methods.length == 0 &&
    ...
    private class ServletHandlerMethodInvoker extends ParancoeHandlerMethodInvoker {
        ...
        public ServletHandlerMethodInvoker(HandlerMethodResolver resolver) {
            super(resolver, webBindingInitializer, sessionAttributeStore,
                    parameterNameDiscoverer, validator, customArgumentResolvers);
        }
        ...
        protected WebDataBinder createBinder(NativeWebRequest webRequest,
            ...
            return ParancoeAnnotationMethodHandlerAdapter.this.createBinder(
                    (HttpServletRequest) webRequest.getNativeRequest(), target,
                    objectName);
        }
        ...
        public ModelAndView getModelAndView(Method handlerMethod,
            ...
            if (returnValue instanceof ModelAndView) {
                ...
                return new ModelAndView((View) returnValue).addAllObjects(
                        implicitModel);
            } else if (returnValue instanceof String) {
                return new ModelAndView((String) returnValue).addAllObjects(
                        implicitModel);
                ...
                ModelAttribute attr = AnnotationUtils.findAnnotation(
                        handlerMethod, ModelAttribute.class);
                String attrName = (attr != null ? attr.value() : "");
                ModelAndView mav = new ModelAndView().addAllObjects(
                        implicitModel);
                if ("".equals(attrName)) {
                    Class resolvedType = GenericTypeResolver.resolveReturnType(
                            handlerMethod, handlerType);
                    attrName = Conventions.getVariableNameForReturnType(
                            handlerMethod, resolvedType, returnValue);
                }
    ...
    private static class RequestMappingInfo {
        ...
        public boolean equals(Object obj) {
            ...
            return (Arrays.equals(this.paths, other.paths) && Arrays.equals(
        ...
        @Override
        public int hashCode() {
            return (Arrays.hashCode(this.paths) * 29 + Arrays.hashCode(
                    this.methods) * 31 +
